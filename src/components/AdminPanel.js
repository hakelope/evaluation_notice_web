...
function AdminPanel() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      if (!formData.id) {
        console.log('수행평가 ID 없음');
        setError('먼저 수행평가를 저장해주세요.');
        return;
      }

      for (let file of files) {
        console.log('이미지 업로드 시도:', { file, evaluationId: formData.id });
    ...
    return (
      <div className="admin-panel">
        ...
        <div className="login-info">
          <p>이 페이지는 수행평가를 추가하고 관리하는 관리자 전용 페이지입니다.</p>
          <p>관리자 인증이 필요합니다.</p>
          <p>선생님이시라면, 3학년 4반 김기륭에게 연락하여 관리자 인증을 받으시고,</p>
          <p>학생이시라면 선생님께 말씀드리거나 오픈채팅으로 연락 주세요.</p>
        </div>
